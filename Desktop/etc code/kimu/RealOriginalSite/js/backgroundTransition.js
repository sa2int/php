    ...
    BackgroundTransition.prototype = {
        ...
        prepareMarkup: function() {
            ...
            $(this.elem)
                ...
                .css('background-image', 'linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5))');//background-image setting
        },
        ...
        loadNext: function() {
            ...
            $('<img/>').attr('id', this.config.idNameDownloadImage).load(function() {
                deferred.resolve();
            }).attr('src', this.config.backgrounds[this.config.imageKey].src).prependTo('body .conA');//class name
            deferred.done(function() {
                setTimeout(self.replaceImage, (self.config.transitionDelay * 300)); //image change speed
            });
        },
