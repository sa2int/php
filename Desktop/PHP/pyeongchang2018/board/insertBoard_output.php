...
$sql=$pdo->prepare('insert into olympicsBoard(b_boardNum, b_subject, b_contents, file01, mem_num) values(null, ?, ?, ?, ?)');
...
if ($sql->execute(
	[htmlspecialchars($_REQUEST['b_subject']), $_REQUEST['b_contents'], $newfile01, $_SESSION['member']['mem_num']]
)) {
	echo '<script> alert("追加に成功しました。"); </script>';
  echo '<meta http-equiv="refresh" content="0.5; url=/test/paku/pyeongchang2018/board/select_olmpicsBoard.php">';
} else {
	echo '<script> alert("追加に失敗しました。"); </script>history.go(-1)';
}
?>
